...
def app():
    ...
    null_count = []
    na_count = []
    for i, col in zip(range(len(numerical_cols.columns)), numerical_cols):
        null_count.append(numerical_cols[col].isnull().sum())
        na_count.append(numerical_cols[col].isna().sum())
        #nan_count = np.isnan(numerical_cols[col].any()).sum()
    missing_values = sum(null_count) #+ sum(na_count)
    if missing_values == 0:
        st.write('Your data has no missing values! Wow, that is rare.')

    else:
        st.write('Your data has ', missing_values, ' missing values.')
        ...
        fig, missing_bar_fig = plt.subplots()
        missing_bar_fig = msno.bar(numerical_cols)
        st.pyplot(fig)
